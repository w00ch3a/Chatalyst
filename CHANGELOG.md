...
- Narrowed the ChatGPT stop-generation selector so sidebar conversations containing the word Stop do not block live sends.
- Fixed workspace-scoped MCP process detection so cleanup only targets the exact requested workspace.
- Fixed existing tag reuse so conversation tag links use the persisted normalized tag ID.
- Refreshed the threat model and security best practices reports for the current release posture.
- Cleaned release artifacts and validated source plus unpacked wheel/sdist privacy scans.
