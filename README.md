...
This release refreshes the release security reports, fixes workspace-scoped MCP
process cleanup, fixes existing tag reuse in the SQLite cache, and narrows the
ChatGPT stop-generation selector. See [CHANGELOG.md](CHANGELOG.md) for the full
release notes.
...
`chatalyst_health` and doctor also report duplicate Chatalyst MCP processes. Use
`uv run chatalyst --kill-extra-processes` on the host to terminate extras for
that workspace.
