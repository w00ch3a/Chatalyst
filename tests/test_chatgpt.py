...
class FakeSelectorPage(FakeProjectPage):
    def __init__(self, visible_by_selector: dict[str, bool]) -> None:
        self.visible_by_selector = visible_by_selector

    def locator(self, selector: str) -> FakeLocator:
        return FakeLocator(self.visible_by_selector.get(selector, False))


@pytest.mark.asyncio
async def test_stop_button_selector_ignores_sidebar_titles_containing_stop(tmp_path):
    ...
    sidebar_only = FakeSelectorPage(
        {
            'button[aria-label*="Stop"]': True,
            'button[aria-label="Unpin StopSlop GitHub Inquiry"]': True,
            'button[aria-label="Open conversation options for StopSlop GitHub Inquiry"]': True,
        }
    )
    actual_stop = FakeSelectorPage({'button[data-testid="stop-button"]': True})

    try:
        assert not await service._any_visible(sidebar_only, service.selectors.stop_button)
        assert await service._any_visible(actual_stop, service.selectors.stop_button)
    finally:
        cache.close()
...
@pytest.mark.asyncio
async def test_extract_projects_normalizes_chatgpt_icon_only_project_rows(tmp_path):
    config = AppConfig.from_workspace(tmp_path, browser_mode="provider")
    cache = ChatCache(config.database_path)
    cache.initialize()
    page = FakeProjectPage()
    page.projects_payload = [
        {"href": "", "name": "Open project home", "aria": "Open project home"},
        {
            "href": "",
            "name": "Open project options for CGPT",
            "aria": "Open project options for CGPT",
        },
        {"href": "", "name": "Projects", "aria": ""},
        {"href": "", "name": "New project", "aria": "New project"},
    ]
    service = ChatGPTService(config, FakeProjectBrowser(page), cache)  # type: ignore[arg-type]
    try:
        projects = await service.extract_projects(page)
    ...
    assert [(project.id, project.name) for project in projects] == [
        (service._hash_id("CGPT"), "CGPT"),  # noqa: SLF001
    ]
