...
def test_chatalyst_version_flag_reports_package_version():
    ...
    assert result.stdout.strip() == "chatalyst 0.3.3"
...
def test_chatalyst_mcp_version_flag_reports_package_version():
    ...
    assert result.stdout.strip() == "chatalyst-mcp 0.3.3"
    assert result.stderr == ""
...
def test_chatalyst_kill_workspace_mcp_processes_reports_payload(tmp_path):
    result = subprocess.run(
        [
            sys.executable,
            "-m",
    ...
    payload = json.loads(result.stdout.replace("'", '"'))
