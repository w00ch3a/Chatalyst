...
class ChatGPTService:
    ...
    async def extract_projects(self, page: Page) -> list[Project]:
        ...
        for item in raw_projects:
            ...
            name = self._project_name_from_ui_label(name)
            if not name:
                continue
    ...
    def _project_name_from_ui_label(self, label: str) -> str:
        ...
        if lower in blocked:
            return ""
        prefix = "open project options for "
    ...
    async def _open_project(self, page: Page, project_name: str) -> None:
        ...
        if project_url:
            ...
            if self._is_chatgpt_app_reference(project_url):
                ...
                await self._wait_for_any(page, self.selectors.composer)
            return
        await self._expand_sidebar_sections(page)
        await self._wait_for_any(page, self.selectors.project_labels)
        if await self._click_visible_project_by_name(page, normalized):
            try:
    ...
    async def _expand_sidebar_sections(self, page: Page) -> None:
        for selector in ('button:has-text("More")', '[aria-label="More"]'):
            try:
                more = page.locator(selector).first
                if await more.is_visible(timeout=750):
                    await more.click()
    ...
    async def _click_visible_project_by_name(self, page: Page, project_name: str) -> bool:
        return bool(
            await page.evaluate(
                """
                (projectName) => {
                    ...
                    const controls = Array.from(document.querySelectorAll(
                        'a, button, [role="button"], [role="treeitem"], [data-testid*="project"]'
                    ));
                    const exactText = controls.find((node) => (
                        textOf(node).toLowerCase() === wanted
                    ));
                    if (exactText) {
                        exactText.click();
                        return true;
                    }
                    const option = controls.find((node) => {
                        const label = (node.getAttribute('aria-label') || textOf(node))
                            .replace(/\\s+/g, ' ')
                            .trim()
                            .toLowerCase();
                        return label === `${optionPrefix}${wanted}`;
                    });
                    if (!option) return false;
                    let row = option.parentElement;
                    for (let depth = 0; row && depth < 8; depth += 1, row = row.parentElement) {
                        const home = Array.from(row.querySelectorAll('a, button, [role="button"]'))
                            .find((node) => {
                                ...
                                const label = (node.getAttribute('aria-label') || textOf(node))
                                    ...
                                    .trim()
                                    .toLowerCase();
                                return label === 'open project home'
                                    || label === wanted
                                    || node.href?.includes('/g/');
                            });
                        if (home) {
                            home.click();
                            return true;
