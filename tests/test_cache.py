...
from chatalyst.core.models import Conversation, Message, MessageRole, Note, Project, SyncStatus, Tag
...
def test_apply_existing_tag_to_another_conversation_uses_real_tag_id(tmp_path) -> None:
    cache = ChatCache(tmp_path / "chat_cache.db")
    cache.initialize()
    cache.upsert_conversation(Conversation(id="c1", title="First"))
    cache.upsert_conversation(Conversation(id="c2", title="Second"))
    cache.apply_tag("c1", Tag(name="release-proof"))
    cache.upsert_note(Note(conversation_id="c2", body="changes sqlite lastrowid"))

    cache.apply_tag("c2", Tag(name="release-proof"))

    assert [tag.name for tag in cache.list_tags("c2")] == ["release-proof"]
